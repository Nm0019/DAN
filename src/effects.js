...
function pointerPrototype(){
    ...
    this.down=true;
    this.moved=false;
  this.color = generateColor(); // شیء {r,g,b}
}
...
if (!GL_OK) {
  ...
  if (ctx2d) {
    ...
    ctx2d.fillStyle = '#fff';
    ctx2d.fillText('WebGL not supported', 20, 30);
  }
  // توقف
  throw new Error('Cannot continue without WebGL');
}
...
function captureScreenshot(){
    ...
    let nid=i*dyeWidth*6+4*j;newPixels[nid+0]=255*clamp01(pixels[id+0]);
    newPixels[nid+1]=255*clamp01(pixels[id+1]),newPixels[nid+2]=255*clamp01(pixels[id+2]);
    newPixels[nid+3]=255*clamp01(pixels[id+3]),id+=4
    }
    let captureCanvas=document.createElement("canvas");
    ctx=captureCanvas.getContext("2d");
    captureCanvas.width=dyeWidth,captureCanvas.height=dyeHeight;
    let imageData=ctx.createImageData(dyeWidth,dyeHeight);
    imageData.data.set(newPixels),ctx.putImageData(imageData,0,0);
    let datauri=captureCanvas.toDataURL();
    downloadURI("fluid.png",datauri),URL.revokeObjectURL(datauri)
    }
...
function render(target){null==target?(gl.blendFunc(gl.ONE,gl.ONE_MINUS_SRC_ALPHA),gl.enable(gl.BLEND)):gl.disable(gl.BLEND);let width=null==target?gl.drawingBufferWidth:dyeWidth,height=null==target?gl.drawingBufferHeight:dyeHeight;if(gl.viewport(0,0,width,height),colorProgram.bind(),blit(target),config.SHADING){let program=displayShadingProgram;program.bind(),gl.uniform2f(program.uniforms.texelSize,1/width,1/height),gl.uniform1i(program.uniforms.uTexture,density.read.attach(0))}else{let program=displayProgram;program.bind(),gl.uniform1i(program.uniforms.uTexture,density.read.attach(0))}blit(target)}
...
function resizeCanvas(){
    canvas.width==canvas.clientWidth&&canvas.height==canvas.clientHeight||(canvas.width=canvas.clientWidth,canvas.height=canvas.clientHeight,initFramebuffers())}function generateColor(){let c=HSVtoRGB(Math.random(),1,1);return c.r*=.15,c.g*=.15,c.b*=.15,c}

function HSVtoRGB(h,s,v){let r,g,b,i,f,p,q,t;switch(i=Math.floor(6*h),f=6*h-i,p=v*(1-s),q=v*(1-f*s),t=v*(1-(1-f)*s),i%6){case 0:r=v,g=t,b=p;break;case 1:r=q,g=v,b=p;break;case 2:r=p,g=v,b=t;break;case 3:r=p,g=q,b=v;break;case 4:r=t,g=p,b=v;break;case 5:r=v,g=p,b=q}return{r:r,g:g,b:b}}

function getResolution(resolution){let aspectRatio=gl.drawingBufferWidth/gl.drawingBufferHeight;aspectRatio<1&&(aspectRatio=1/aspectRatio);let max=Math.round(resolution*aspectRatio),min=Math.round(resolution);return gl.drawingBufferWidth>gl.drawingBufferHeight?{width:max,height:min}:{width:min,height:max}}

function getTextureScale(texture,width,height){return{x:width/texture.width,y:height/texture.height}}update(),document.body.addEventListener("mousemove",(e=>{const x=e.clientX,y=e.clientY;pointers[0].moved=pointers[0].down,pointers[0].dx=5*(x-pointers[0].x),pointers[0].dy=5*(y-pointers[0].y),pointers[0].x=x,pointers[0].y=y})),document.body.addEventListener("touchmove",(e=>{const touches=e.targetTouches;for(let i=0;i<touches.length;i++){let pointer=pointers[i];pointer.moved=pointer.down,pointer.dx=8*(touches[i].clientX-pointer.x),pointer.dy=8*(touches[i].clientY-pointer.y),pointer.x=touches[i].clientX,pointer.y=touches[i].clientY}}),!1),document.body.addEventListener("touchstart",(e=>{const touches=e.targetTouches;for(let i=0;i<touches.length;i++)i>=pointers.length&&pointers.push(new pointerPrototype),pointers[i].id=touches[i].identifier,pointers[i].down=!0,pointers[i].x=touches[i].clientX,pointers[i].y=touches[i].clientY,pointers[i].color=generateColor()}));

window.addEventListener("mouseover",(()=>{pointers[0].down=!0,pointers[0].color=generateColor()}));

window.addEventListener("touchend",(e=>{const touches=e.changedTouches;for(let i=0;i<touches.length;i++)for(let j=0;j<pointers.length;j++)touches[i].identifier==pointers[j].id&&(pointers[j].down=!1)}));